...
excerpt: ActivityWatch's macOS dev releases have been broken for weeks. The fix required
  four pull requests over one day, each uncovering a deeper layer of macOS code signing
  complexity. Here's the full debugging trail — from 'codesign --deep' to inside-out
  bundle signing to Python.framework structure quirks.
---
